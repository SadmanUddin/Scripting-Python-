#You must create a Python script called script.py that checks the validity of a Belgian National Register Number entered by the user. The script should verify that the number follows the correct format (YY.MM.DD-XXX.CC), contains only digits, represents a real and non-future date, and has valid check digits based on official rules (adding “2” for people born after 2000). It should also identify gender (odd = male, even = female), registration type (early or late), and generation based on the birth year. If the number is valid, the user can choose to display the information as structured text, generate a QR code, or view popular names from that period; if invalid, the script must specify the reason (e.g., incorrect format, invalid date, wrong check digits, etc.).
